...
func TestRedisControlUpdateRedis(t *testing.T) {
	...
	type testcase struct {
		name                             string
		update                           func(redi *v1alpha1.Redis)
	...
	tests := []testcase{
		...
		},
		{
			name: "normal",
			update: func(redi *v1alpha1.Redis) {
				redi.Status.Replica.MasterName = "demo-redis-0"
				redi.Status.Replica.StatefulSet = &apps.StatefulSetStatus{ReadyReplicas: 2}
			},
			syncReclaimPolicyErr:             false,
			syncReplicaMemberManagerErr:      false,
			syncRedisClusterMemberManagerErr: false,
...
func newFakeRedisControl() (ControlInterface, *meta.FakeReclaimPolicyManager, *mm.FakeReplicaMemberManager, *mm.FakeRedisClusterMemberManager, *meta.FakeMetaManager) {
	...
	rcInformer := informers.NewSharedInformerFactory(cli, 0).Anywhy().V1alpha1().Redises()
...
func newRedisForRedisControl() *v1alpha1.Redis {
