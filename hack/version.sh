...
REDIS_VERSION=${REDIS_VERSION:-5.0.3}
...
function redis_operator::version::get_version_vars() {
...
function redis_operator::version::ldflag() {
  local key=${1}
  local val=${2}
...
function redis_operator::version::ldflags() {
  redis_operator::version::get_version_vars

  local buildDate=
  [[ -z ${SOURCE_DATE_EPOCH-} ]] || buildDate="--date=@${SOURCE_DATE_EPOCH}"
  local -a ldflags=($(redis_operator::version::ldflag "buildDate" "$(date ${buildDate} -u +'%Y-%m-%dT%H:%M:%SZ')"))
  if [[ -n ${GIT_COMMIT-} ]]; then
    ldflags+=($(redis_operator::version::ldflag "gitCommit" "${GIT_COMMIT}"))
    ldflags+=($(redis_operator::version::ldflag "gitTreeState" "${GIT_TREE_STATE}"))
  fi

  if [[ -n ${GIT_VERSION-} ]]; then
    ldflags+=($(redis_operator::version::ldflag "gitVersion" "${GIT_VERSION}"))
  fi
...
redis_operator::version::ldflags
